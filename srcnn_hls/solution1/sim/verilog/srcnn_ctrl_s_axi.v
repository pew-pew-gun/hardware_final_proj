...
#(parameter
    ...
    output wire [0:0]                    reload_weights,
...
// int_reload_weights[0:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_reload_weights[0:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_RELOAD_WEIGHTS_DATA_0)
            int_reload_weights[0:0] <= (WDATA[31:0] & wmask) | (int_reload_weights[0:0] & ~wmask);
    end
end
